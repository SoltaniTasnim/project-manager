...
export class HighlightStatusDirective implements OnChanges {
  ...
  ngOnChanges(): void {
    ...
    if (this.status === 'En attente') {
      this.renderer.setStyle(this.el.nativeElement, 'backgroundColor', '#FEF9C3'); // ~ yellow-100
    } else if (this.status === 'En cours') {
      this.renderer.setStyle(this.el.nativeElement, 'backgroundColor', '#DBEAFE'); // ~ blue-100
    } else if (this.status === 'Terminé') {
      this.renderer.setStyle(this.el.nativeElement, 'backgroundColor', '#DCFCE7'); // ~ green-100
    }
  }
}
